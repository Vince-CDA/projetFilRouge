...
if (
    ...
    if ((isset($_GET['filename'])) && (preg_match("/^[^\/\"'<>*]+$/", $_GET['filename']))) {
        ...
        if ($files_count >= 1) {
            ...
            while ($i < $files_count) {
                if ($files["$i"] == $requested_file) {
                    ...
                    $file_name = $files["$i"];
                    $thumb_file_location = 'gallery/thumbnails/' . $requested_category . '/thumb-' . $file_name;
                    $source_file_location = $requested_category . '/' . $file_name;
                    $category_items .= '<li><div><a href="index.php?page=viewer&category='.$requested_category.'&filename='.$file_name.'"><img src="'.$thumb_file_location.'" alt="'.$file_name.'"></a></div></li>';
                }
                ++$i;
            }
            $category_items .= '</ul>';
        ...
        if ($previous_file !== false) {
            $html_content .= '<div id="previous" class="p"><a href="index.php?page=viewer&category='.$requested_category.'&filename='.$previous_file.'">&lt;</a></div>';
        }
        if ($next_file !== false) {
            $html_content .= '<div id="next"><a href="index.php?page=viewer&category='.$requested_category.'&filename='.$next_file.'">&gt;</a></div>';
        }
        
        $html_content .= '<img src="'.$path_to_file.'" alt="'.$requested_file.'">';
        $html_action_controls = '<div id="action_controls"><ul>
<li><a href="index.php?page=galerie&category='.$requested_category.'">Back</a></li>
</ul>
